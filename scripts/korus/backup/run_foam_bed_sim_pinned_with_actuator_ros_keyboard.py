...
def run_simulator(sim: sim_utils.SimulationContext, entities, ros_node: Node, keyctl: KeyControl):
    """
    Simulation loop:
      - Pins the deformable cube's bottom-corner vertices to the rigid base's top corners.
      - Publishes the foam's top-surface Z grid and XYZ over ROS2.
      - Drives the inflatable cell's prismatic joint from the keyboard:
          Up/W   -> inflate  (+ step)
          Down/S -> deflate  (- step)
          Space  -> stop     (no change)
    """
    ...
    nodal_kinematic_target = cube.data.nodal_kinematic_target.clone()  # (B,N,4) -> [x,y,z,flag], 0=constrained, 1=free
    ...
    # --- Joint command state (set after limits known) ---
    q_cmd = None  # (B_cell,) tensor for the prismatic dof
    q_min = None
    q_max = None
    ...
    while simulation_app.is_running():
        ...
        if count % 10000 == 0:
            ...
            # --- Build surface sets / corner pins / top grid (from default state) ---
            z_top_default = round(FOAM_ORIGIN[2] + FOAM_SIZE[2] / 2, 2)
            z_bot_default = round(FOAM_ORIGIN[2] - FOAM_SIZE[2] / 2, 2)

            top_surface_idx_torch    = get_surface_indices_by_known_z(cube, z_target=z_top_default, decimals=DECIMALS)
            bottom_surface_idx_torch = get_surface_indices_by_known_z(cube, z_target=z_bot_default, decimals=DECIMALS)
            ...
            try:
                if hasattr(inflatable_cell.data, "joint_pos_limits"):
                    lims = inflatable_cell.data.joint_pos_limits  # (B,DoF,2)
                    if lims.ndim == 3 and lims.shape[-1] == 2:
                        lower_limits = lims[..., 0]
                        upper_limits = lims[..., 1]
                elif hasattr(inflatable_cell.data, "joint_limits"):
                    lims = inflatable_cell.data.joint_limits  # (B,DoF,2)
                    if lims.ndim == 3 and lims.shape[-1] == 2:
                        lower_limits = lims[..., 0]
                        upper_limits = lims[..., 1]
            except Exception:
...
def main():
    ...
    try:
        run_simulator(sim, entities, ros_node, keyctl)  # pass keyctl through
    finally:
        keyctl.unsubscribe()
