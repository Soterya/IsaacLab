...
def design_scene(cfg: AppCfg):
    ...
    # NOTE & TODO: Actually Tuning Stiffness Parameters is better done using GUI and then incorporated into the scripts/korus/utils/convert_xml1_to_xml2_joint_limits.py
    ...
    humanoid_cfg = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0, 1.5, 3.0),
            rot=(1, 0, 0, 0),
            joint_pos=JOINT_CONFIG,# {".*": 0.0},   # zero all hinge joints by default
            joint_vel={".*": 0.0},
        ),
...
def run_simulator(sim: sim_utils.SimulationContext, entities, ros_node: Node, keyctl: KeyControl, cfg: AppCfg):
    ...
    while simulation_app.is_running():
        ...
        if count % RESET_AFTER == 0:
            ...
            # joints: NPZ pose as both state and PD target
            jpos_h = q_target_hum.clone()
            jvel_h = torch.zeros_like(jpos_h)
            humanoid.write_joint_state_to_sim(jpos_h, jvel_h)
            humanoid.reset()

            # important: tell the PD controller that NPZ pose is the desired target
