...
Per-image (per-tile) normalization examples:
  python3 view_all_pressure_grids.py --rows 4 --cols 8 --per_image_norm
  python3 view_all_pressure_grids.py --rows 4 --cols 8 --per_image_norm --p_lo 5 --p_hi 95
"""
...
def normalize_tile_01(
    ...
    """
    Robust per-tile normalization to [0,1] using percentiles.
    Returns (g01, lo, hi) where lo/hi are the chosen percentile bounds in original units.
    """
    finite = np.isfinite(g)
    if not finite.any():
        return np.zeros_like(g, dtype=np.float32), 0.0, 1.0

    lo = float(np.nanpercentile(g[finite], p_lo))
    hi = float(np.nanpercentile(g[finite], p_hi))
    ...
    g01 = (g - lo) / (hi - lo)
    g01 = np.clip(g01, 0.0, 1.0).astype(np.float32)
    return g01, lo, hi
...
class AllPressureViewer(Node):
    ...
    def snapshot(self) -> List[List[Optional[np.ndarray]]]:
        with self._lock:
            return [
                [
                    None if self._latest[r][c] is None else self._latest[r][c].copy()
...
def main():
    ...
    ap.add_argument(
        "--prefix",
        type=str,
        default="/foam_bed/top_surface/pressure_grid_",
    ...
    # ---- NEW: per-image normalization controls ----
    ap.add_argument(
        "--per_image_norm",
        action="store_true",
        help="Normalize each tile independently to 0..1 for visualization (shared colorbar shows normalized units).",
    )
    ap.add_argument(
        "--p_lo",
        type=float,
        default=1.0,
        help="Low percentile for robust per-image scaling (default 1).",
    )
    ap.add_argument(
        "--p_hi",
        type=float,
        default=99.0,
        help="High percentile for robust per-image scaling (default 99).",
    )
    ...
    if args.per_image_norm:
        # Shared colorbar shows normalized units (0..1)
        norm = Normalize(vmin=0.0, vmax=1.0)
        autoscale = False
    else:
        # Your existing behavior: fixed global range OR global autoscale across all tiles
    ...
    if args.per_image_norm:
        cbar.set_label("Normalized (per-tile) 0..1")
    else:
    ...
    try:
        while plt.fignum_exists(fig.number):
            ...
            # Global autoscale across all received tiles (robust to NaN/inf)
            if autoscale:
                mins, maxs = [], []
                for r in range(args.rows):
                    for c in range(args.cols):
            ...
            for r in range(args.rows):
                for c in range(args.cols):
                    ...
                    if args.per_image_norm:
                        g_show, lo, hi = normalize_tile_01(g, p_lo=args.p_lo, p_hi=args.p_hi)
                        im.set_data(g_show)  # type: ignore
                        axes[r][c].set_title(f"r={r}, c={c}  [{lo:.2f}, {hi:.2f}]")
                        H, W = g_show.shape
                    else:
