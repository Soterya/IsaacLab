...
Usage:
    ...
    $ preload ./isaaclab.sh -p scripts/korus/run_korus_bed_classes_rigid_plates_articulation_pose_multiple_with_record.py \
        ...
        --npz_dir scripts/korus/assets/npz \
        --record_dir outputs/korus_mocap

NOTE:
- This records (per episode / per pose):
  1) MoCap in WORLD frame:
     - root position [1 : (x,y,z)] + root rotation vector [1 - (rx, ry, rz)] (pelvis position)
     - body link positions [24 : (x,y,z)] + body link rotvecs [1 - (rx, ry, rz)]
  2) "pose_body" in ROTVEC form (same format as SMPL .npz):
     - one (3,) rotvec per SMPL joint in SMPL_BODY_JOINT_ORDER
  3) Pressure maps:
     - pressure[t, cell, H, W] saved in the same episode file

- Recording is done every `--every` frames (same as ROS publish cadence).
"""
...
parser.add_argument("--every"       , type=int  , default=1                                     , help="ROS publish + record every N frames")
...
args_cli.experience = "isaacsim.exp.full.kit" # NOTE: Necessary for ROS2 TODO: Remove during data generation, since you won't be using ros2 while saving data.  
...
def save_episode_npz(
    ...
    root_pos = np.stack(root_pos_list, axis=0).astype(np.float32)       # [T, 3] - root pos (TODO: currently the spine, need it to be pelvis)
    root_rotvec = np.stack(root_rotvec_list, axis=0).astype(np.float32) # [T, 3] - root rotvec (TODO: currently the spine, need it to be pelvis)
...
def design_scene(cfg: AppCfg, initial_joint_config: dict[str, float]):
    ...
    # import the humanoid
    humanoid_asset = sim_utils.UsdFileCfg(usd_path=cfg.humanoid_usd, scale=(4.0, 4.0, 4.0))
...
def run_simulator(
    ...
    """
    - Publishes ONLY pressure grids (per row/col topic)
    - Drives the bed’s prismatic joints via keyboard and ROS (ROSBedController)
    - Records:
        * Root world pose (pos + rotvec)
        * Body link world poses (pos + rotvec)
        * pose_body in rotvec format (SMPL joint order)
        * pressure[t, cell, H, W]
    """
    bed      = entities["bed"]
    ...
    body_names = humanoid.data.body_names

    pose_idx = 0
    q_target_hum: Optional[torch.Tensor] = None
    current_npz: Optional[str] = None
    ...
    # publishers (ONLY pressure now)
    pubs_press = []  # [pub_pressure_rc]

    # per-cell caches still needed for pinning + pressure
    bottom_corner_idx = []
    kin_buffers = []
    valid_mask_list = []
    tri_maps_list = []
    ...
    # make per-cell pressure pubs
    for ent in cells:
        r = ent["row"]
        c = ent["col"]
        pub_pressure_rc = ros_node.create_publisher(
            Float32MultiArray, f"/foam_bed/top_surface/pressure_grid_{r}_{c}", 10
        )
        ...
        tri_maps_list.append(None)
        HW_press_list.append((0, 0))
    ...
    while simulation_app.is_running():
        ...
        if count % RESET_AFTER == 0:
            ...
            jpos_h = q_target_hum.clone()  # type: ignore
        ...
        if q_target_hum is not None and (count % args_cli.every == 0):
            episode_time_s.append(float(sim_time))

            root_pose_w_numpy = humanoid.data.root_pose_w[0].detach().cpu().numpy()
            root_pos       = root_pose_w_numpy[:3]
            root_quat_wxyz = root_pose_w_numpy[3:]
            root_rotvec = quat_wxyz_to_rotvec(root_quat_wxyz)
            episode_root_pos.append(root_pos.astype(np.float32))
            episode_root_rotvec.append(root_rotvec.astype(np.float32))

            body_pos  = humanoid.data.body_link_pos_w[0].detach().cpu().numpy().astype(np.float32)     # [Nb,3]
            body_quat = humanoid.data.body_link_quat_w[0].detach().cpu().numpy().astype(np.float32)  # [Nb,4] wxyz
            Nb = body_pos.shape[0]
        ...
        # Collect + publish pressure per-frame (aligned with `--every`)
        ...
        for n, ent in enumerate(cells):
            ...
            pos_np = bed.data.body_link_pos_w[0, body_id].detach().cpu().numpy()
            ...
            P_t = PressureMapper.compute_pressure_grid_torch(
                tri_nodes_t, tri_elem_t, cell_ids_t,
                S_valid_t, sim_pos_t, Hm, Wm
            )

            # record pressure grid per cell
            frame_pressure_cells.append(P_t.detach().cpu().numpy().astype(np.float32))
