#!/usr/bin/env python3
import os
import math
import argparse
import xml.etree.ElementTree as ET

# ---------------- config ----------------
# Force this exact meshdir in the top <compiler />
MESH_DIR_TARGET = "asset/smpl_meshes/geom"

# Anatomical joint limits in **degrees** TODO: Make sure to write a clip function in record joint angles since an error can be thrown if input to the joint angles is more than the limits
# (you can tweak these later if needed)
JOINT_LIMITS_DEG = {
    "L_Hip" :    {"x": (-120,  30), "y": (-30,  45), "z": (-15 ,  45)},
    "R_Hip" :    {"x": (-120,  30), "y": (-45,  30), "z": (-45 ,  15)},
    "Torso" :    {"x": ( -25,  25), "y": (-15,  15), "z": (-25 ,  25)},
    "L_Knee":    {"x": (-10,  135), "y": ( -5,   5), "z": ( -5 ,   5)},
    "R_Knee":    {"x": (-10,  135), "y": ( -5,   5), "z": ( -5 ,   5)},
    "Spine":     {"x": ( -30,  30), "y": (-20,  20), "z": (-30 ,  30)},
    "L_Ankle":   {"x": ( -30,  45), "y": (-20,  20), "z": (-15 ,  15)},
    "R_Ankle":   {"x": ( -30,  45), "y": (-20,  20), "z": (-15 ,  15)},
    "Chest":     {"x": ( -20,  20), "y": (-15,  15), "z": (-25 ,  25)},
    "L_Toe":     {"x": ( -10,  10), "y": (-10,  10), "z": (-10 ,  10)},
    "R_Toe":     {"x": ( -10,  10), "y": (-10,  10), "z": (-10 ,  10)},
    "Neck":      {"x": ( -40,  50), "y": (-35,  35), "z": (-70 ,  70)},
    "L_Thorax":  {"x": ( -45,  45), "y": (-30,  30), "z": (-40 ,  40)},
    "R_Thorax":  {"x": ( -45,  45), "y": (-30,  30), "z": (-40 ,  40)},
    "Head":      {"x": ( -30,  30), "y": (-25,  25), "z": (-50 ,  50)},
    "L_Shoulder":{"x": ( -90,  90), "y": (-135, 30), "z": (-40 , 120)},
    "R_Shoulder":{"x": ( -90,  90), "y": (-30, 135), "z": (-120, 40)},
    "L_Elbow":   {"x": ( -5,    5), "y": ( -135, 5), "z": ( -5 ,   5)},
    "R_Elbow":   {"x": ( -5,    5), "y": ( -5, 135), "z": ( -5 ,   5)},
    "L_Wrist":   {"x": ( -20,  45), "y": (-70,  70), "z": (-30 ,  30)},
    "R_Wrist":   {"x": ( -20,  45), "y": (-70,  70), "z": (-30 ,  30)},
    "L_Hand":    {"x": ( -45,  45), "y": (-20,  20), "z": (-20 ,  20)},
    "R_Hand":    {"x": ( -45,  45), "y": (-20,  20), "z": (-20 ,  20)},
}

# ----------------------
# --- NOTE: FALLBACK ---
# ----------------------
# JOINT_LIMITS_DEG = {
#     "Spine":     {"x": (-30,  30), "y": (-20,  20), "z": (-30,  30)},
#     "Torso":     {"x": (-25,  25), "y": (-15,  15), "z": (-25,  25)},
#     "Chest":     {"x": (-20,  20), "y": (-15,  15), "z": (-25,  25)},
#     "Neck":      {"x": (-40,  50), "y": (-35,  35), "z": (-70,  70)},
#     "Head":      {"x": (-30,  30), "y": (-25,  25), "z": (-50,  50)},
#     "L_Thorax":  {"x": (-30,  60), "y": (-30,  30), "z": (-40,  40)},
#     "R_Thorax":  {"x": (-30,  60), "y": (-30,  30), "z": (-40,  40)},
#     "L_Shoulder":{"x": (-60, 140), "y": (-40, 150), "z": (-90,  90)},
#     "R_Shoulder":{"x": (-60, 140), "y": (-40, 150), "z": (-90,  90)},
#     "L_Elbow":   {"x": ( -5, 145), "y": ( -5,   5), "z": ( -5,   5)},
#     "R_Elbow":   {"x": ( -5, 145), "y": ( -5,   5), "z": ( -5,   5)},
#     "L_Wrist":   {"x": (-70,  70), "y": (-30,  45), "z": (-30,  30)},
#     "R_Wrist":   {"x": (-70,  70), "y": (-45,  30), "z": (-30,  30)},
#     "L_Hand":    {"x": (-45,  45), "y": (-20,  20), "z": (-20,  20)},
#     "R_Hand":    {"x": (-45,  45), "y": (-20,  20), "z": (-20,  20)},
#     "L_Hip":     {"x": (-120, 30), "y": (-30,  45), "z": (-30,  30)},
#     "R_Hip":     {"x": (-120, 30), "y": (-45,  30), "z": (-30,  30)},
#     "L_Knee":    {"x": ( -135, 5), "y": ( -5,   5), "z": ( -5,   5)},
#     "R_Knee":    {"x": ( -135, 5), "y": ( -5,   5), "z": ( -5,   5)},
#     "L_Ankle":   {"x": (-30,  45), "y": (-20,  20), "z": (-15,  15)},
#     "R_Ankle":   {"x": (-30,  45), "y": (-20,  20), "z": (-15,  15)},
#     "L_Toe":     {"x": (-10,  80), "y": (-10,  10), "z": (-10,  10)},
#     "R_Toe":     {"x": (-10,  80), "y": (-10,  10), "z": (-10,  10)},
# }
# ---------------- helpers ----------------

def find_or_create(parent, tag):
    el = parent.find(tag)
    if el is None:
        el = ET.SubElement(parent, tag)
    return el

def rewrite_mesh_files_to_basenames(asset):
    if asset is None:
        return
    for m in asset.findall("mesh"):
        f = m.get("file")
        if f:
            m.set("file", os.path.basename(f))  # strip any directories

def ensure_top_compiler_with_forced_meshdir(root):
    comp = root.find("compiler")
    if comp is None:
        comp = ET.Element("compiler")
        root.insert(0, comp)
    comp.set("coordinate", "local")
    comp.set("meshdir", MESH_DIR_TARGET)
    # leave angle unset here (defaults to degrees)

def ensure_mesh_geom_contact_attrs(geom):
    if geom.get("type") == "mesh":
        geom.set("contype", "1")
        geom.set("conaffinity", "1")
        geom.set("condim", "3")
        geom.set("margin", "0.001")

def get_joint_limits_deg(joint_name):
    """
    joint_name: e.g. 'L_Hip_x'
    returns (lo_deg, hi_deg) or None if not in table
    """
    if "_" not in joint_name:
        return None
    base, axis = joint_name.rsplit("_", 1)
    axis = axis.lower()
    base_limits = JOINT_LIMITS_DEG.get(base)
    if base_limits is None:
        return None
    return base_limits.get(axis)

def update_joint_attributes(j):
    name = j.get("name", "")

    # 1) Set anatomical limits if we have them
    limits = get_joint_limits_deg(name)
    if limits is not None:
        lo, hi = limits
        j.set("range", f"{lo:.4f} {hi:.4f}")
        j.set("limited", "true")
    else:
        # If there is an existing range, keep it (still degrees), just mark limited
        r = j.get("range")
        if r is not None:
            j.set("range", r)
            j.set("limited", "true")

    # 2) Generic dynamic properties
    # (Pelvis freejoint has no range but can still get these attributes; that's fine)
    j.set("armature", "0.1")
    if j.get("damping") is None:
        j.set("damping", "0.01")
    j.set("stiffness", "10")

def remove_world_floor_and_light(worldbody):
    to_remove = []
    for elem in list(worldbody):
        if elem.tag == "geom" and elem.get("type") == "plane":
            to_remove.append(elem)
        elif elem.tag == "light":
            to_remove.append(elem)
    for e in to_remove:
        worldbody.remove(e)

def set_pelvis_root(worldbody):
    pelvis = None
    for b in worldbody.findall("body"):
        if b.get("name") == "Pelvis":
            pelvis = b
            break
    if pelvis is None:
        return
    pelvis.set("pos", "0 0 1.0")
    if pelvis.find("freejoint") is None:
        fj = ET.SubElement(pelvis, "freejoint")
        fj.set("name", "Pelvis")

def apply_to_all_joints(elem):
    for j in elem.iter("joint"):
        update_joint_attributes(j)

def ensure_mesh_geom_contacts_everywhere(root):
    for g in root.iter("geom"):
        ensure_mesh_geom_contact_attrs(g)

def add_model2_defaults(root):
    dflt = root.find("default")
    if dflt is None:
        dflt = ET.SubElement(root, "default")

    # generic joint defaults
    j1 = ET.SubElement(dflt, "joint")
    j1.set("damping", "0.01")
    j1.set("armature", "0.1")
    j1.set("stiffness", "10.0")
    j1.set("limited", "true")

    # generic geom defaults
    g = ET.SubElement(dflt, "geom")
    g.set("conaffinity", "1")
    g.set("condim", "3")
    g.set("contype", "7")
    g.set("margin", "0.001")
    g.set("rgba", "0.8 0.6 .4 1")
    g.set("type", "box")
    g.set("size", "0.01 0.01 0.01")

    # extra joint default for convenience
    j2 = ET.SubElement(dflt, "joint")
    j2.set("damping", "1")
    j2.set("armature", "0.1")

def add_tail_compiler_and_stat(root):
    # keep "angle" in **degrees**, but still use autolimits
    tail_comp = ET.Element("compiler")
    tail_comp.set("angle", "degree")
    tail_comp.set("autolimits", "true")
    root.append(tail_comp)

    stat = ET.Element("statistic")
    stat.set("meansize", "0.05")
    root.append(stat)

def add_ctrlrange_to_motors(root, ctrlrange="-200 200"):
    act = root.find("actuator")
    if act is None:
        return
    for m in act.findall("motor"):
        m.set("ctrlrange", ctrlrange)

def pretty_write(tree, out_path):
    tree.write(out_path, encoding="utf-8", xml_declaration=True)

# ---------------- main ----------------
def main():
    ap = argparse.ArgumentParser(description="Convert MuJoCo XML (style 1) to style 2, with anatomical joint limits in degrees.")
    ap.add_argument("input_xml", help="Path to input model-1 XML")
    ap.add_argument("output_xml", help="Path to write model-2-like XML")
    args = ap.parse_args()

    tree = ET.parse(args.input_xml)
    root = tree.getroot()

    # Force meshdir and strip mesh file paths to basenames
    rewrite_mesh_files_to_basenames(root.find("asset"))
    ensure_top_compiler_with_forced_meshdir(root)

    # Defaults like model (2)
    add_model2_defaults(root)

    # World edits
    worldbody = root.find("worldbody")
    if worldbody is not None:
        remove_world_floor_and_light(worldbody)
        set_pelvis_root(worldbody)

    # Mesh geoms + joints
    ensure_mesh_geom_contacts_everywhere(root)
    apply_to_all_joints(root)

    # Actuators ctrlrange like model (2)
    add_ctrlrange_to_motors(root, ctrlrange="-200 200")

    # Trailing compiler/statistic (now in degrees)
    add_tail_compiler_and_stat(root)

    pretty_write(tree, args.output_xml)
    print(f"Wrote converted XML to: {args.output_xml}")

if __name__ == "__main__":
    main()
