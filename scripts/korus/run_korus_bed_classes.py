...
def make_multiarray_2d(arr2d: np.ndarray) -> Float32MultiArray:
    """
    NOTE: this is for publishing as ros2 topics
    """
...
def design_scene(cfg: AppCfg):
    """
    Build world + a single KorusBed articulation + (rows x cols) cells. 
    Returns dict with 'bed' and 'cells' (each cell has cube/base/sphere row/col/index).
    """
    ...
    humanoid_asset = sim_utils.UsdFileCfg(usd_path=cfg.humanoid_usd, scale=(4.,4.,4.)) # TODO: remove the scaling after dimensionality fix; currently scaled x4.5 
    ...
    # define articulation for the bed NOTE: this requires an `Articulation Root` at the Base Plate. 
        ...
        for c in range(cfg.cols):
            ...
            base_cfg = RigidObjectCfg(
                ...
                init_state=RigidObjectCfg.InitialStateCfg(
                    pos=(cfg.base_origin[0] + x_off, cfg.base_origin[1] + y_off, cfg.base_origin[2])
                ),
            )
            ...
            # sphere (NOTE: optional: for debugging)
            ...
            #     ),
            #     init_state=RigidObjectCfg.InitialStateCfg(
            #         pos=(cfg.sphere_origin[0] + x_off, cfg.sphere_origin[1] + y_off, cfg.sphere_origin[2])
            #     ),
            # )
...
def run_simulator(sim: sim_utils.SimulationContext, entities, ros_node: Node, keyctl: KeyControl, cfg: AppCfg):
    ...
    while simulation_app.is_running():
        ...
        if count % 4000 == 0:
            ...
            print(f"[INFO] Reset bed + {len(cells)} cells; prismatic joints = {len(ros_ctl.pris_ids)}")
        ...
        for n, ent in enumerate(cells):
            ...
            # keep bottom corners pinned (reuse buffer)
            root_pos  = np.round(base.data.root_pos_w.detach().cpu().numpy(), 3)
            root_quat = np.round(base.data.root_quat_w.detach().cpu().numpy(), 3)

            # compute rigid base corners (top face)
            half_x = round(cfg.base_size[0] / 2.0, 2)
            half_y = round(cfg.base_size[1] / 2.0, 2)
            half_z = round(cfg.base_size[2] / 2.0, 2)
            pins = []
            for b in range(root_pos.shape[0]):
                ...
                pins.append(corners)
            targets_b = torch.tensor(pins, dtype=cube.data.nodal_kinematic_target.dtype,
